...
// ============= Your Code Here =============
type String2Union<S, R = never> = S extends `${infer F}${infer Rest}` ? String2Union<Rest, F | R> : R;

type AllCombinations<S, T extends string = String2Union<S>, U extends string = T> =
  [T] extends [never] ? '':
  T extends U
    ? T | `${T}${AllCombinations<S, Exclude<U, T>>}` | ''
    : ''