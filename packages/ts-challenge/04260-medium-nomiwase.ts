...
type String2Union<T extends string> = T extends `${infer F}${infer Rest}` ? F | String2Union<Rest> : ''

type AllCombinations<S extends string, T extends string = String2Union<S>,  E extends string = T> =
  T extends E
    ? '' | T | `${T}${AllCombinations<S, Exclude<E, T>>}`
    : never